...
try:
	fileLootTables = open("output/loot/lootTables.hpp", "w")
	fileItemGroups = open("output/loot/itemGroups.hpp", "w")
except IOError:
		exit()

fileLootTables.write("	class LootTables\n")
fileLootTables.write("	{\n")
fileItemGroups.write("	class ItemGroups\n")
fileItemGroups.write("	{\n")

# EncodedTableHelper is the category name
# dividedLootTables is the 10-separated 2d array
# lootTables is the precursor to dividedLootTables

for i in range(10):
	fileLootTables.write("		" + str(EncodedTableHelper[i].name) + "[] = {")

	j = 1;
	for item in dividedLootTables[i]:
		fileItemGroups.write("		" + str(EncodedTableHelper[i].name) + str(j) + "[] = {")
		output = ""
		for itemTwo in item:
			output += "\"" + itemTwo + "\", "
		output = output[:-2]
		fileItemGroups.write(output + "};\n")
		j += 1
	fileItemGroups.write("\n")
	...
	output = output[:-2]
	fileLootTables.write(output + "};\n")

fileLootTables.write("	}\n")
fileItemGroups.write("	}\n")

fileLootTables.close()
fileItemGroups.close()
